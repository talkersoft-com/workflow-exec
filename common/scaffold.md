# Workflow Scaffold Instructions

Follow these steps in order to create a new workflow from scratch.
Read this file top to bottom before creating any files.
...
Check the workflow folder for existing files. If none exist, use `0001`. If a prior run exists (e.g. files prefixed `0001-`), use `0002`. The number is shared across every file in a single run.
...
## Branch
*(filled in after Task 0000 runs)*

## Initialize (Task 0000)
`‌``
hv_status  deck: "<deck-name>"
hv_init    deck: "<deck-name>"
`‌``
If already provisioned on a prior feature branch with all PRs merged:
`‌``
hv_status  deck: "<deck-name>"
hv_next    deck: "<deck-name>"
`‌``

## Ship
`‌``
hv_ship  deck: "<deck-name>"
