...
## Manual steps
**vorch redeploy:** the cloud-manager deploy script (`deploy-cloud-manager.py`) targets api/mcp/web
only. `vorch-service` runs on the hypervisor host and must be rebuilt + restarted there after merge.
Document the exact build/restart command used in `Results/RESULT.md` during exec.
