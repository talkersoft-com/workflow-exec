...
- [x] `Tasks/0000-TASK.md` — **Phase 0**: hv_status; confirm `agile-conch` across all repos
- [x] `Tasks/0001-TASK.md` — **Phase 1**: porch extravars — inject `run_public_id`, fix `secretsPrefix`; rebuild + restart porch
- [x] `Tasks/0002-TASK.md` — **Phase 2**: VAULT_ADDR FQDN — `envvars` emits the configured `https://ubuntu-server.talkersoft.com:8200`
- [x] `Tasks/0003-TASK.md` — **Phase 3**: extend `cloud_playbook_update` (API + MCP) if content patches aren't supported yet
- [x] `Tasks/0004-TASK.md` — **Phase 4**: revert `pg-14-jammy` + `pg-16-noble` playbooks via `cloud_playbook_update` — back to `{{ run_public_id }}`, drop the six dev-only patches
- [x] `Tasks/0005-TASK.md` — **Phase 5**: `cloud-manager-api/scripts/vault/bootstrap-policies.py` — writes `playbook-run` policy + role; updates `cloudmanager-playbook-runner` read path
- [x] `Tasks/0006-TASK.md` — **Phase 6**: `cloud-manager-mcp/.cicd/export-playbooks.py` + `import-playbooks.py`; commit `cloud-manager-api/seed/playbooks/`
- [x] `Tasks/0007-TASK.md` — **Phase 7**: `bootstrap-worker.py` calls `bootstrap-policies.py` + `import-playbooks.py` after `hv init`
- [x] `Tasks/0008-TASK.md` — **Phase 8**: E2E verify — fresh playbook run end-to-end, no manual SQL or Vault CLI ops
- [~] `Tasks/0009-TASK.md` — **Phase 9**: teardown + rebootstrap verify — full `bootstrap-server.py` cycle ends green — skipped — destructive rebootstrap deferred to a separate workflow
- [x] `Tasks/0010-TASK.md` — **Phase 10**: write `Results/RESULT.md` + `Retro/LESSONS.md` (enumerate the six `fix_*.sql` patches and which phase replaced each); `hv_ship`
