...
7. Series-order gate (init.md constraint): verify P1 and P2 are merged to main —
   inventory layers (`gvar`/`hvar`) and `vblk` variable blocks with `defines`/`consumed_by`
   edges must exist on origin/main (entities + migrations in cloud-manager-api). If any
...
Prerequisites (P1, P2) confirmed merged.
